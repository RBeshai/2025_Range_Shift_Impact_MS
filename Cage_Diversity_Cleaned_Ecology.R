...
CMS_AS_Evenness_Summary <-summary(CMS_AsEvenness)
...
# Define the model 
Global_MexShannon_lm <- lmer(formula = Change_Shannon_Mex ~ RunAvg_Whelks * Region + (1|Site), data = filter(DiversityMex, Plot_ID %!in% c("No Cage", "Partial") & Weeks_Deployed == 8))
  # Fit is singular because of zero variance attributed to site. 
  # Keeping in the model for consistency
...
Global_MexRich_lm <- lmer(formula = Change_Richness_Mex ~ RunAvg_Whelks * Region + (1|Site), data = filter(DiversityMex, Plot_ID %!in% c("No Cage", "Partial") & Weeks_Deployed == 8))
...
Global_MexEven_lm <- lmer(formula = Change_Evenness_Mex ~ RunAvg_Whelks * Region + (1|Site), data = filter(DiversityMex, Plot_ID %!in% c("No Cage", "Partial") & Weeks_Deployed == 8))
...
Dana_AS_Richness_Summary 
Scripps_Richness_Summary 

# Site-level Acanthinucella on Evenness 
CM_AS_Evenness_Summary
CMS_AS_Evenness_Summary 
Dana_AS_Evenness_Summary 
Scripps_Evenness_Summary
...
rm(CM_AS_Richness_Summary, CMS_AS_Richness_Summary, Dana_AS_Richness_Summary, Scripps_Richness_Summary)
rm(CM_AS_Evenness_Summary, CMS_AS_Evenness_Summary, Dana_AS_Evenness_Summary, Scripps_Evenness_Summary)
